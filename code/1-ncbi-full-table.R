...
write.csv(mv, here("data/marine-vert-refg-ncbi.csv"), row.names = F)
write.csv(wv.species, here("results/marine-vert-species.csv"), row.names = F)



## Summarise the number of marine species per order
all.orders <- wv.species |> group_by(class, order) |> tally() |> arrange(desc(n))

## Remove duplicate species (9 species have two assemblies)
ncbi.orders <- mv |> filter(!duplicated(`Organism Name`))
ncbi.orders <- mv |> group_by(class,order) |> tally() |> arrange(desc(n))
